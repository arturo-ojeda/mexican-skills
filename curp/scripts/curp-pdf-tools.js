...
const PDF_NAME_PREFIX = process.env.CURP_PDF_NAME_PREFIX || 'CURP';
const PDF_NAME_SLUG = process.env.CURP_PDF_NAME_SLUG || 'curp';
...
function savePdfBuffer(body, meta = {}, runDir, artifactsDir = DEFAULT_ARTIFACTS_DIR) {
  ...
  const capturedPath = path.join(runDir, `curp-raw-${nowStamp()}.pdf`);
  ...
  return {
    ...
    source: meta.source || 'agent_browser_eval',
  };
}
